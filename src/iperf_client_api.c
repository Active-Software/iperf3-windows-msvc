...
#ifdef _WIN32
                /*
                 * Native Winsock peers can otherwise observe the data stream
                 * shutdown as a reset at the end of a sender test.  Send a FIN
                 * on each data stream before the control TEST_END message so
                 * non-Windows servers can finish receiving and report results.
                 */
                SLIST_FOREACH(sp, &test->streams, streams) {
                    if (sp->sender && sp->socket > -1) {
